...
export class CadastrarAtividadeMedica {
  ...
  public cadastrar() {
    ...
    const atividadeMedicaModel: CadastrarAtividadeMedicaModel = {
      ...
      inicio: new Date(this.inicio?.value).toISOString(),
      termino: new Date(this.termino?.value).toISOString(),
      medicos:
        typeof this.medicos?.value === 'string' ? [this.medicos?.value] : [...this.medicos?.value],
    };

    const cadastroObserver: Observer<CadastrarAtividadeMedicaResponseModel> = {
      next: () =>
        this.notificacaoService.sucesso(
          `A atividade médica com início em ${atividadeMedicaModel.inicio} foi cadastrada com sucesso!`
        ),
