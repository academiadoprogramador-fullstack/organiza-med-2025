...
export class EditarPaciente {
  ...
  public editar() {
    ...
    const pacienteModel: EditarPacienteModel = this.pacienteForm.value;

    const edicaoObserver: Observer<EditarPacienteResponseModel> = {
      next: () =>
        this.notificacaoService.sucesso(
          `O registro "${pacienteModel.nome}" foi editado com sucesso!`
        ),
    ...
    this.paciente$
      .pipe(
        take(1),
        switchMap((paciente) => this.pacienteService.editar(paciente.id, pacienteModel))
      )
